...
from utils import date_from_filename, date_to_filename, get_config, save_activity_log
...
def get_activity_id(function_name, function_callargs):
    return "%s.%s-%s" % (
        function_name,
        date_to_filename(date_from_filename(function_callargs["untracked_maps"][0])),
        date_to_filename(date_from_filename(function_callargs["untracked_maps"][-1])),
    )
...
def get_tracked_map(tracked_maps, untracked_maps, config):
    ...
    logging.info(
        "Running tracking on region maps\n%s\n%s",
        "\n".join("Tracked - %s" % m for m in tracked_maps),
        "\n".join("Untracked - %s" % m for m in untracked_maps),
    )

    job = Job(
        config.get("executable"),
        positional_parameters=tracked_maps + untracked_maps,
        optional_parameters={"config": config.get("config_file")},
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "-c",
        required=True,
        help="Path to the config file of the script",
    )
    parser.add_argument(
        "--tracked-map",
        metavar="FILEPATH",
        action="append",
        default=[],
        help="The path to a previously tracked region map to establish tracking relations with the past",
    )
    parser.add_argument(
        "untracked_maps",
        metavar="FILEPATH",
        nargs="+",
        help="The path to an untracked region map",
    )
    ...
    try:
        get_tracked_map(
            args.tracked_map, args.untracked_maps, config["GET_TRACKED_MAP"]
        )
